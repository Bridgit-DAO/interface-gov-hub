...
def test_production_uses_mainnet_client_by_default(monkeypatch):
    ...
    monkeypatch.delenv('WEB3AUTH_USE_DEVNET', raising=False)
    ...
    assert settings['google_verifier'] == 'web3auth-google-sapphire'


def test_production_devnet_requires_explicit_override(monkeypatch):
    from services import web3auth_config
    ...
    monkeypatch.setenv('WEB3AUTH_CLIENT_ID', mainnet)
    monkeypatch.setenv('WEB3AUTH_CLIENT_ID_DEVNET', devnet)
    monkeypatch.setenv('WEB3AUTH_USE_DEVNET', 'true')
    monkeypatch.setattr(web3auth_config, 'IS_DEVELOPMENT', False, raising=False)
